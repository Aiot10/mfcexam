...
BEGIN_MESSAGE_MAP(CMFCcircleDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTN_IMAGE, &CMFCcircleDlg::OnBnClickedBtnImage)
	ON_BN_CLICKED(IDC_BTN_ACTION, &CMFCcircleDlg::OnBnClickedBtnAction)
END_MESSAGE_MAP()
...
void CMFCcircleDlg::OnBnClickedBtnImage()
{
...
void CMFCcircleDlg::OnBnClickedBtnAction()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	for (int i = 0; i < 640; i++)
	{
...
BOOL CMFCcircleDlg::validimgPos(int x, int y)
{
	int nWidth = m_image.GetWidth();
	int nHeight = m_image.GetHeight();
...
void CMFCcircleDlg::drawCircle(unsigned char* fm, int x, int y, int nRadius, int nGray)
{
...
bool CMFCcircleDlg::isinCircle(int i,int j,int nCenterX,int nCenterY,int nRadius)
{
	bool bRet = false;
